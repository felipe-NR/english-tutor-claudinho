...
// Translate a Claude Code hook payload into the normalized shape. Claude Code
// identifies the turn with prompt_id. Returns undefined for a payload that does
// not parse, so the runner emits the client's empty output.
export function parse(event: HookEvent, raw: JsonValue): HookInput | undefined {
  switch (event) {
    ...
    case "user-prompt-submit": {
      ...
      const turn = parsed.data.prompt_id ?? parsed.data.session_id;
